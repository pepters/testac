...
public class testclass implements PacketListener {
    ...
    public void onPacketReceive(PacketReceiveEvent event) {
        ...
        if (event.getPacketType() == PacketType.Play.Client.ANIMATION) {
            ...
            StorageManager.getPlayer(player.getUUID()).startswing();
        }
        if (event.getPacketType() == PacketType.Play.Client.INTERACT_ENTITY) {
            WrapperPlayClientInteractEntity packet = new WrapperPlayClientInteractEntity(event);
            if (packet.getAction() != WrapperPlayClientInteractEntity.InteractAction.ATTACK) return;
            if (StorageManager.getPlayer(player.getUUID()).hasSwung()) {
                logger.info("Player hasnt swung");
            }
            StorageManager.getPlayer(player.getUUID()).finishswing();
        }
